...
This theme now uses [Brevo](https://www.brevo.com/) to deliver table-booking confirmations by email and SMS. Customers receive a link to verify their booking; once clicked, the booking is marked as confirmed and both the customer and admin receive final confirmation emails.
...
1. Go to **PixelForge Options → Brevo Email & SMS** and fill in:
   ...
   - **SMS Sender ID**: the approved alphanumeric sender ID from Brevo (up to 11 characters).
...
- Ensure your API key has permission for transactional email and SMS (Brevo → **SMTP & API**).
- Register/approve your SMS sender ID and ensure SMS credits are available.
...
- If Brevo is misconfigured, the system falls back to WordPress mail for emails; SMS will silently skip if the sender ID, API key, or phone number is missing.
